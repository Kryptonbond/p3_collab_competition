...
WEIGHT_DECAY = 0.0 # L2 weight decay ## Saurabh it was 0 earlier which i made to 0.1
EPSILON = 0.1
NUM_AGENTS = 2
UPDATE_SIZE = 10
...
class Agent():
    ...
    def __init__(self, state_size, action_size, num_agents, random_seed):
        ...
        #self.noise = [OUNoise(action_size, random_seed) for i in range(self.num_agents)] #Sam
...
#                     j += self.epsilon * self.noise.sample()   #self.noise[i].sample()  #Sam
#         return np.clip(action, -1, 1)
#     def reset(self):
#         for i in range(self.num_agents):
#             self.noise.reset()                     #self.noise.reset() #Sam
    ...
    def learn(self, experiences, gamma):
        ...
        Q_targets = rewards + (GAMMA * Q_targets_next * (1 - dones))  #gamma ##Sam 
        # Compute critic loss
        ...
        torch.nn.utils.clip_grad_norm_(self.critic_local.parameters(), 1)     ##Sam check to comment
...
class ReplayBuffer:
    ...
    def __init__(self, action_size, buffer_size, batch_size, seed):
        ...
        self.memory = deque(maxlen=buffer_size)  # internal memory (deque) saur_abh
